...
** working with magit
  I could not figure out how to commit files.  It seems most of the magit commands must be invoked in the special buffer magit uses to display status. That is once the special magit buffer is up then typing a '?' will show the available commands.  
