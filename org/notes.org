...
** Run the install files in this order
   ...
   *note* now that I have rebooted it does appear to show the proper version number.
   ...
   Now that npm is running install the various libraries needed for HTML and JavaScript development.  I had to symlink each executable directly into ~/bin/. I need to come up with something better.
   npm install -g tern
